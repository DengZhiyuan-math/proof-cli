...
@/Users/zhdeng/Proof CLI /.planning/milestones/1.0-phases/07-publication-grade-proof-pipelines/07-CONTEXT.md
@/Users/zhdeng/Proof CLI /.planning/milestones/1.0-phases/07-publication-grade-proof-pipelines/07-RESEARCH.md
...
<task type="auto">
  ...
  <files>src/proof_cli/publication.py, src/proof_cli/storage.py, src/proof_cli/theorems.py, src/proof_cli/collaboration.py, src/proof_cli/commands.py, src/proof_cli/cli.py, tests/test_publication.py</files>
  <read_first>/Users/zhdeng/Proof CLI /.planning/milestones/1.0-phases/07-publication-grade-proof-pipelines/07-CONTEXT.md</read_first>
  <action>Create publication-view and publication-state models for theorem-like claims, selection rules for paper-facing versus supplement-facing versus internal-only artifacts, summary rendering for readiness reasons, and CLI commands to inspect and set publication state. Keep publication readiness distinct from theorem trust state and collaboration approval.</action>
...
<task type="auto">
  <name>Validate real project delivery and bundle round-tripping</name>
  <files>tests/test_publication.py, tests/test_export.py, tests/test_exchange.py, tests/test_cli.py</files>
  <read_first>/Users/zhdeng/Proof CLI /.planning/milestones/1.0-phases/07-publication-grade-proof-pipelines/07-CONTEXT.md</read_first>
  <action>Build an end-to-end validation on a real mathematical section or theorem cluster, ideally the higher-rank radial system section or a similarly rich project slice. Verify that the project can produce a paper-facing export, a supplement-facing export, and a reproducible bundle; that internal-only artifacts remain suppressed; and that the bundle can be inspected or handed off without losing provenance or release state.</action>
  <verify>pytest tests/test_publication.py tests/test_export.py tests/test_exchange.py tests/test_cli.py -q</verify>
  <acceptance_criteria>
    - a real project section can be moved from internal proof workspace to publication package
    - publication outputs preserve trust boundaries and release history
    - the workflow demonstrates clear reduction in friction for research delivery
  </acceptance_criteria>
  <done>Real-project publication validation passes</done>
</task>
