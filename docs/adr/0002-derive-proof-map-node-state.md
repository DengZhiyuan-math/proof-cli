...
**Update (ADR-0004):** `accepted` and `rejected` moved out of this single table onto their own axis (acceptance state), separate from the workflow-progress states here (open/claimed/review-needed/revision-requested/blocked). ADR-0004 also adds a third axis, integrity state, for the Challenge mechanism. The rule that `blocked` never overrides `accepted`/`rejected` in what's displayed still holds — it was, and remains, about ordering within the workflow axis — but that rule no longer needs to explain a state (`accepted`) that has since moved to a different axis entirely. Nothing above this note is invalidated by that split; the derive-don't-store principle applies to all three axes alike.

**Update (ADR-0006):** "a review decision on that candidate proof ends the claim" is tightened to "submitting the candidate proof ends the claim" — a review decision always comes strictly after submission, and workflow state already stops showing `claimed` the moment a candidate proof is awaiting review, so the claim was already over by then regardless. This is a precision fix, not a change in what happens.
